...
sqlite3* openDB(){
    ...
    int rc;
//    FILE *conf_file;
//    char* file_name = "../dbName.txt";
//    bool flag = true;
//    if((conf_file= fopen(file_name, "r"))==NULL)
//    {
//        perror("Error occured while opening conf file");
//        flag = false;
//    }
//    char* db_Name = (char*) malloc(sizeof(char)*100);
//    if(flag == false){
//        strcpy(db_Name, "testServ.db");
//    }
//    else{
//        fscanf(conf_file, "%s", db_Name);
//    }
//    fclose(conf_file);
    rc = sqlite3_open("testServ.db", &db);
    if(rc){
...
bool configurateDB(char* name_db){
...
bool addNewMess_db(char* reciever, char* mess){
    ...
    int rc;
    sqlite3_stmt* statement;
    sprintf(sql, "INSERT INTO messages (sender, receiver, message) VALUES('unknown', '%s', '%s')",
            reciever, mess);
...
UserList* get_list_diologs_db(){
    ...
    int k = 0;
    sqlite3_stmt* statement;
    sprintf(sql, "SELECT DISTINCT receiver FROM messages");
    unsigned int len = strlen(sql);
    rc = sqlite3_prepare_v2(db, sql, (int)len, &statement, NULL);
    if(rc == SQLITE_OK){
        while(sqlite3_step(statement) == SQLITE_ROW){
